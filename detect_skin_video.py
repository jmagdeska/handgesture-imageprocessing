...
def draw_circle(event,x,y,flags,param):
    ...
    if event == cv2.EVENT_LBUTTONDBLCLK:
        mouseX,mouseY = x,y
        image = cv2.imread("frames_initials/initial_" + letters[l] + "-1.png")
...
for l in range(10):
    ...
    cap = cv2.VideoCapture('C:\Users\Jana\PyCharmProjects\skin-detection/videos/gesture-' + letters[l] + '-1.avi')
    ...
    cap.set(1, 2)
    ret, frame = cap.read()
    cv2.imwrite("frames_initials/initial_" + letters[l] + "-1.png", frame)

    for i in range(2,75,3):
        cap.set(1, i)
        ret, frame = cap.read()
        cv2.imwrite("frames/initial_" + letters[l] + "-1-" + format(k) + ".png", frame)
        k += 1

for l in range(10):
    print("Select skin color initial frame of letter " + letters[l])
    image_initial = cv2.imread("frames_initials/initial_" + letters[l] + "-1.png")
    ...
    for i in range(1,25):
        a = cv2.imread("frames/initial_" + letters[l] + "-1-" + format(i) + ".png")
        ...
        cv2.imwrite("frames_skin/" + letters[l] + "-1-" + format(i) + ".png", blank_image)
